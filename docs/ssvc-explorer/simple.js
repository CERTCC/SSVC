...
function get_decision_point(name, version, namespace) {
    ...
    if(name in SSVC.dpMap && !version) {
	version = SSVC.dpMap[name]["version"]
	namespace = SSVC.dpMap[name]["namespace"];
	/* Check if name is remapped in CSVs*/	
	if("name" in SSVC.dpMap[name]) 
	    name = SSVC.dpMap[name]["name"];
    }
    if(!version)
	version = "1.0.0";
    if(!namespace)
	namespace = "ssvc";
    for(let i = 0; i < SSVC.decision_points.length; i++) {
	if(SSVC.decision_points[i].data.name == name &&
	   SSVC.decision_points[i].data.namespace == namespace &&
	   SSVC.decision_points[i].data.version == version) {
	    return SSVC.decision_points[i].data;
	}
    }
    return {};
}
function update_stats() {
	...
	if(outcome in SSVC.results )  {
	    if(el.parentElement.style.display != "none")
		SSVC.results[outcome] += 1;
	...
	rlabel.innerText = " (" + String(SSVC.results[outcome]) + ")";
	let dbar = document.createElement("span");
	dbar.innerHTML = "&nbsp;";
	dbar.style.marginLeft = "6px";
	dbar.style.display = "inline-block";
	dbar.style.width = String(parseInt(70 * SSVC.results[outcome]/outcomeMax)) + "px";
...
function createSSVC(csv, uploaded) {
	...
	jsonTree = JSON.parse(JSON.stringify(csv));
    ...
    const form = SSVC.form;
    const main = document.createElement("main");
    function destroytip() {
	let div = SSVC.form.querySelector("[data-temp]");
	if(div)
	    div.remove();
    }
	...
	headers.forEach(function(dpName,i) {
	    while(dpName in SSVC.dpMap) {
		let idx = 1;
		dpName = dpName + "-" + idx;
		idx = idx + 1;
	    }
	    headers[i] = dpName;
	    SSVC.dpMap[dpName] = {"namespace": "x_com.example#psirt",
				  "version": "1.0.0"};
	});
    }
	...
	SSVC.decision_points.push.apply(SSVC.decision_points,
					Object.values(newdps));
    }
    
    SSVC.decision_table = ssvcTable;
    const numberTable = toNumberTable(SSVC.decision_table,Object.keys(SSVC.decision_table[0]));
    const features = [];
    const results = []
    numberTable.forEach(function(row) {
	const outcome = row.pop();
	if(SSVC.results[outcome])
	    SSVC.results[outcome] = SSVC.results[outcome] + 1;
	else
	    SSVC.results[outcome] = 1;
	results.push(outcome);
	features.push(row);
    });
    if(Object.keys(SSVC.results).length > 1) {
	const labels = SSVC.decision_table.map(function(x) {
...
function loadSSVC(fileurl) {
	...
	SSVC.form.parentElement.querySelector("input[type='file']").click();
	return;
    }
	...
	if( index in SSVC.decision_trees && SSVC.decision_trees[index].data) {
	    ...
	    return createSSVC(SSVC.decision_trees[index].data);
	}
    }
...
async function get_decision_points() {
    ...
    const response = await fetch("https://raw.githubusercontent.com/CERTCC/SSVC/refs/heads/main/data/json/ssvc_object_registry.json");
    ...
    SSVC.decision_points.sort(dtreeSort); 
    load_trees();
}
...
function popupStart(selector) {
    ...
    popup.previousElementSibling.style.opacity = "0.3";
    popup.previousElementSibling.style.pointerEvents = "none";
...
function disable_current_dps(options) {
    ...
    const current_dps = [];
    SSVC.form.querySelector("main").querySelectorAll("[data-dp]").forEach(function(el) {
	try {
...
function popupEditDP(w) {
	...
	prepare_form(dpForm, dpSelect, {}, SSVC.decision_points, "values");
	options = dpSelect.querySelectorAll("option");
    } else if(SSVC.decision_points.length > options.length - 1) {
...
function toggleAll(doselect) {
    const main = SSVC.form.querySelector('main');
...
function selectCustom(name, datacsv, fIndex) {
    ...
    if(fIndex < 0) {
	let opt;
	if(typeof(datacsv) === "object")
	    opt = new Option(name, "json:" + datacsv, false, true);
	else
	    opt = new Option(name, "csv:" + datacsv, false, true);	    
	if(name) {
	    opt.text = name;
	    opt.selected = true;
	}
	...
	sample.querySelectorAll("option").forEach(function(option) {
		...
		if(typeof(datacsv) === "object")
		    option.value = "json:" + datacsv;
		else
		    option.value = "csv:" + datacsv;
	    }
	});
    }
    toggleAll(true);
...
function customize(w) {
	...
	const divOutcome = SSVC.form.querySelector("[data-outcomeName]");
	const span = document.createElement("span");
	span.innerHTML = "&#9998;";
	...
	if(findex > -1) {
	    ...
	    SSVC.decision_trees.push({filename: "csv:" + datacsv, displayname: current});
	}
	selectCustom(current, datacsv, findex);	
...
async function delete_session() {
	...
	load_trees();
	SSVC.form.parentElement.querySelector("[data-session]").style.display = "none";	
    } else {
...
function computeFI(features,labels) {
    ...
    featureNames = Object.keys(SSVC.decision_table[0]);
    featureNames.pop()
