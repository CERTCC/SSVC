...
def initialize_options():
  parser = optparse.OptionParser(description = """Take a CSV file as input
	 that describes a decision tree exhaustively, and produce a LaTeX
         file that will make a pretty version of the tree.""")
  parser.add_option("-i", "--input", dest='filelist', type='string', \
    default='stdin', help="input file(s). may be a glob, but will run all \
    output together. Defaults to standard input.")
  parser.add_option("-o", "--output", dest='out_file', type='string',\
    default = 'stdout', help="Defaults to stdout.")
  parser.add_option("-d", "--delim", default="	", help="identifies delimiter\
    for the input data (--input). May be any number of characters. Tab default")
  ##########
  ## put other option definitions here
  parser.add_option("-l", "--label", default="5", type ='int',
  ...
  parser.add_option("-c", "--columns", default="1,2,3,4", type ='string',
  ...
  parser.add_option("--header-row", action='store_true',
    dest='headerRow',
  ...
  parser.add_option("-H", "--no-header-row", action='store_false',
    dest='headerRow',
    help="Boolean indicating whether the input file has a header row with names\
  ...
  parser.add_option("-p", "--priorities", type='string',
...
def print_forest_options(location, priorities = ["Defer", "Scheduled",
  "Out-of-band","Immediate"]):
  """
...
def main():
  ...
  delim = options.delim
  if delim is not None: delimlen = len(delim)
  ...
  while i >= 0:
    ...
    if i == depth - 1: #off by one error if we just test == depth
      ...
      for j in range(len(dpoint_values[i])):
        ...
        ofile.write("[, " + graph[tuple(tmp_path)] + ", my label={" + label + "} ]" + "\n")
        del tmp_path[-1]
      i = i - 1
      ...
      if counts[i] == len(dpoint_values[i]):
        try:
          ...
          tmp_path = [] # basically this just allows us to reach the loop exit
        ...
        tmp_path.append(label)
        outstring = str("[" + names[i+1] + ", rectangle, draw, my label={" +
          label + "}," +"\n")
  ...
  ofile.close()
  print("""If everything went OK, then you should be able to create a PDF from \
