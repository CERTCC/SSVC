...
from ssvc.decision_tables import experimental_base as base
...
class TestDecisionTable(unittest.TestCase):
    ...
    def test_load_from_csv_str(self):
        ...
        print("Original CSV String:")
        print(csv_str)
        # modify the csv_str to simulate a new mapping
        # replace "OV2" with "OV1"
        modified_csv_str = csv_str.replace("OV2", "OV1")
        # confirm that "OV2" is no longer in the modified string
        self.assertNotIn("OV2", modified_csv_str)
        print("Modified CSV String:")
        print(modified_csv_str)
        # load the modified mapping from the CSV string
        new_dt = base.DecisionTable.load_from_csv_str(csv_str=modified_csv_str)
        self.assertIsInstance(new_dt, base.DecisionTable)
        self.assertEqual(new_dt.name, self.dt.name)
        self.assertEqual(new_dt.description, self.dt.description)
        self.assertEqual(new_dt.namespace, self.dt.namespace)
        self.assertEqual(new_dt.decision_point_group, self.dpg)
        self.assertEqual(new_dt.outcome_group, self.og)
        # check that the mapping is the same as the modified CSV string
        new_mapping = new_dt.get_mapping_df()
        self.assertIsInstance(new_mapping, DataFrame)
        self.assertFalse(new_mapping.empty)
        # check that the modified value is in the mapping
        # for each row in the old mapping, the new mapping should have the same values
        # original: OV0 = new: OV1
        # original: OV1 = new: OV1
        # original: OV2 = new: OV1
        for index, row in df.iterrows():
            print(row)

        self.assertIn("OV1", new_mapping[self.og.str].unique())
