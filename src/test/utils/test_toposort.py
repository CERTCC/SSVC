...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.dp1 = DecisionPoint(
            ...
            values=[
                DecisionPointValue(name="Value 1", key="V1", description="value 1 description"),
                DecisionPointValue(name="Value 2", key="V2", description="value 2 description"),
            ],
        )
        self.dp2 = DecisionPoint(
            ...
            values=[
                DecisionPointValue(name="Value A", key="VA", description="value A description"),
                DecisionPointValue(name="Value B", key="VB", description="value B description"),
            ],
        )
    ...
    def test_graph_from_dplist(self, mock_graph_from_value_tuples):
        ...
        mock_graph_from_value_tuples.assert_called_once_with(
            [tuple(range(len(self.dp1.values))), tuple(range(len(self.dp2.values)))]
        )
    ...
    def test_graph_from_value_tuples(self):
        ...
        self.assertEqual(len(G.nodes), node_count)  # 3! = 6, 2! = 2, 2! = 2, total = 6 + 2 + 2 = 10

        for u,v in itertools.product(G.nodes(), G.nodes()):
            if _off_by_one(_diff(u, v)):
                # edges should point from v to u
                self.assertTrue(G.has_edge(v, u), f"Expected edge from {u} to {v} but it does not exist.")
            else:
                # edges should not point from v to u
                self.assertFalse(G.has_edge(v, u), f"Expected no edge from {u} to {v} but it exists.")
    ...
    def test_tuple_to_dict(self):
        ...
        nodes = list(itertools.product(range(len(self.dp1.values)), range(len(self.dp2.values))))
        for node in nodes:
            node = tuple(node)
    ...
    def test_dplist_to_toposort(self):
        ...
        self.assertTrue(all(isinstance(k, str) and isinstance(v, str) for item in result for k, v in item.items()))
        ...
        expected_lowest = {self.dp1.id: self.dp1.values[0].key, self.dp2.id: self.dp2.values[0].key}
        self.assertEqual(result[0], expected_lowest)

        # highest item should be V2,VB
        expected_highest = {self.dp1.id: self.dp1.values[-1].key, self.dp2.id: self.dp2.values[-1].key}
